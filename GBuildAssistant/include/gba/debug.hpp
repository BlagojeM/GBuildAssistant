...
template<class FormatString, class... Args> void warn(FormatString &&fmt, Args &&...args)
{
  spdlog::warn(std::forward<FormatString>(fmt), std::forward<Args>(args)...);
}

template<class FormatString, class... Args> void info(FormatString &&fmt, Args &&...args)
{
